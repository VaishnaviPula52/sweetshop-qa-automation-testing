...
test.describe('Basket — TC-042 to TC-058', () => {
  ...
  test('TC-042 | Basket page loads with Your Basket heading', async ({ page }) => {
    await expect(page.locator('h1')).toContainText(pageContent.basketTitle);
  });

  test('TC-043 | Empty basket shows 0 items', async ({ page }) => {
    // h4 contains "Your Basket 0" — matches actual DOM
    await expect(page.locator('h4').filter({ hasText: /Your Basket/ }))
      .toContainText('0');
  });

  test('TC-044 | Collect (FREE) delivery option is visible', async ({ page }) => {
    await expect(
      page.locator('label').filter({ hasText: /Collect/ })
    ).toBeVisible();
  });

  test('TC-045 | Standard Shipping (£1.99) option is visible', async ({ page }) => {
    await expect(
      page.locator('label').filter({ hasText: /Standard Shipping/ })
    ).toBeVisible();
  });

  test('TC-046 | Standard Shipping option can be selected', async ({ page }) => {
    const shippingRadio = page.locator('input[type="radio"]').nth(1);
    await shippingRadio.click();
    await expect(shippingRadio).toBeChecked();
  });

  test('TC-047 | Promo code input field is visible', async ({ page }) => {
    const bp = new BasketPage(page);
    await expect(bp.promoInput).toBeVisible();
  });

  test('TC-049 | Empty promo code shows validation message', async ({ page }) => {
  ...
  test('TC-050 | Billing first name required validation fires on submit', async ({ page }) => {
  ...
  test('TC-051 | Billing last name required validation fires', async ({ page }) => {
    const bp = new BasketPage(page);
  ...
  test('TC-054 | Country dropdown is present with United Kingdom option', async ({ page }) => {
    const dropdown = page.locator('select').first();
    await expect(dropdown).toBeVisible();
    await expect(dropdown.locator('option:has-text("United Kingdom")')).toHaveCount(1);
  });

  test('TC-055 | City dropdown has Bristol, Cardiff, Swansea options', async ({ page }) => {
  ...
  test('TC-056 | Payment card name field is visible', async ({ page }) => {
    await expect(
      page.locator('input[placeholder="Full name as displayed on card"]')
    ).toBeVisible();
  });

  test('TC-057 | Payment card number field is visible', async ({ page }) => {
    await expect(
      page.locator('#cc-number, input[placeholder*="card" i]').first()
    ).toBeVisible();
  });

  test('TC-058 | Payment CVV field is visible', async ({ page }) => {
    await expect(
